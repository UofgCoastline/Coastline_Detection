...
def run(
        ...
        source=ROOT / 'football1.mp4',  # file/dir/URL/glob, 0 for webcam
...
def parse_opt():
    ...
    parser.add_argument('--project', default=ROOT / 'results/predict', help='save results to project/name')
    ...
    def get_default_name(source):
        source_path = Path(source)
        # If it's a directory or webcam, use a default name
        if source == '0' or source.isdigit():
            return 'exp'
        elif source_path.is_dir():
            return source_path.name
        # Otherwise, use the filename without extension
        return source_path.stem + '_'

    # If name is not provided, generate it from the source
    if opt.name is None:
        opt.name = get_default_name(opt.source)
